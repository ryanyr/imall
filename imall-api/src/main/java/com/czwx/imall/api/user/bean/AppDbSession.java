package com.czwx.imall.api.user.bean;
...
//	private int liveMin = 60 * 24 * 7;
	...
	public boolean remove(String token) {
		...
		Map rec = dbService.queryRec(
				"select * from im_app_session where token=?", token);
		if (rec == null) {
			return false;
		}

		dbService
				.update("delete from im_app_session where id=?", rec.get("id"));
		return true;
	}
	...
	public Object access(String token) {
		...
		if (!(Boolean) rec.get("status")) {
			dbService.update("delete from im_app_session where token=?", token);
	...
	public AppSessionBean create(HttpServletRequest request, String loginname,
		...
		Map<String, Object> user = dbService.queryRec(
				"select * from im_user where login_name=?", loginname);
		long userId = Long.parseLong(user.get("id").toString());

		// Map customer =
		// dbService.queryRec("select * from im_user_base_info where user_id=?",
		// userId);
		...
		Map oSession = dbService
				.queryRec("select * from im_app_session where user_id="
	...
	public Map<String, Object> getUserData(String token) {
		Map rec = dbService
				.queryRec("select session from im_app_session where token='"
						+ token + "'");
	...
	public Object autoLogin(HttpServletRequest request, String refreshToken) {
		...
		Map rec = dbService.queryRec(
				"select login_name from im_user where uuid=?", refreshToken);
		if (rec == null) {
